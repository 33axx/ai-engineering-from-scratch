...
> 大多数人写提示词就像和朋友发短信一样随意。然后他们奇怪一个 2000 亿参数的模型为什么给出平庸的回答。提示工程不是关于花招。而是关于理解你发送的每一个 token 都是一条指令，而模型会逐字逐句地遵循指令。编写更好的指令，获得更好的输出。就是这么简单又这么难。

**类型：** 构建
**语言：** Python
**前置条件：** 阶段 10，课程 01-05（从头开始构建 LLM）
**时间：** 约 90 分钟
**相关：** 阶段 11 · 05（上下文工程）了解窗口中还应包含哪些内容；阶段 5 · 20（结构化输出）了解 token 级别的格式控制。

## 学习目标

- 应用核心提示工程模式（角色、上下文、约束、输出格式）将模糊的请求转化为精确的指令
- 构建带有显式行为规则的系统提示词，从而产生一致且高质量的输出
- 诊断提示词失败（幻觉、拒绝响应、格式违规）并通过有针对性的提示词修改来修复
- 实现一个提示词测试框架，根据一组预期输出来评估提示词修改的效果

## 问题

你打开 ChatGPT。你输入：“帮我写一封营销邮件。”得到的是泛泛而谈、冗长且不可用的东西。你再试一次，添加更多细节。好了一点，但仍然不对。你花了 20 分钟重新措辞同一个请求。这不是模型的问题。这是指令的问题。
...
第一个提示词激活了模型训练数据中营销邮件的一般性分布。第二个提示词激活了一个狭窄的、高质量的切片。相同的模型。相同的参数。截然不同的输出。

这个“你问什么得到什么”的差距正是提示工程这门学科的全部内容。它不是一个 hack 或变通方法。它是人类意图与机器能力之间的主要接口。它是一个更大学科——上下文工程（在第 05 课中介绍）——的子集，该学科处理进入模型上下文窗口的所有内容，而不仅仅是指示本身。

提示工程没有过时。那些说它已死的人，和 2015 年说 CSS 已死的是同一批人。变化在于它成为了入门级技能。每个严肃的 AI 工程师都需要它。问题不是要不要学，而是学到多深。
...
**系统消息**：无形之手。它设定模型的角色、行为约束和输出规则。模型将其视为最高优先级的上下文。OpenAI、Anthropic 和 Google 都支持系统消息，但它们在内部的处理方式不同。Claude 对系统消息的遵循最严格。GPT-5 在长对话中有时会偏离系统指令，而 Gemini 3 将 `system_instruction` 作为一个独立的生成配置字段而不是消息来处理。

**用户消息**：任务。这就是大多数人认为的“提示词”。但如果没有好的系统消息，用户消息的约束就不够。

**助手预填充**：秘密武器。你可以用一段部分字符串来启动助手的回答。发送 `{"role": "assistant", "content": "```json\n{"}` and the model will continue from there, producing JSON without preamble. Anthropic's API supports this natively. OpenAI does not (use structured outputs instead).
...
LangChain 让你编写一个提示模板并跨提供商运行。这就是跨模型提示设计的实际实现。

## 发布

本课程产生两个输出：

`outputs/prompt-prompt-optimizer.md` —— 一个元提示词，它接受任何草稿提示词并使用本课程中的 10 种模式进行重写。输入一个模糊的提示词，返回一个工程化的提示词。

`outputs/skill-prompt-patterns.md` —— 一个决策框架，用于根据任务类型、所需可靠性和目标模型选择合适的提示模式。

Python 代码（`code/prompt_engineering.py`）是一个独立的测试框架。通过将 `simulate_llm_call` 替换为实际向 OpenAI、Anthropic 和 Google API 发出的 HTTP 请求，即可接入真实的 API 调用。模式库、构建器、评分器和比较逻辑无需修改即可工作。

## 练习

1. 获取 `TEST_SUITE` 中的 5 个测试用例，并额外添加 5 个覆盖其余模式（元提示、分解、批评、受众适应、边界）的用例。运行完整套件，并确定哪种模式在所有模型上产生最一致的分数。

2. 将 `simulate_llm_call` 替换为至少两个提供商（OpenAI 和 Anthropic 的免费层可使用）的实际 API 调用。在两者上运行相同的提示，并测量：响应长度、格式合规性、关键词覆盖率和延迟。记录哪个模型更精确地遵循指令。

3. 构建一个提示注入测试套件。编写 10 个试图覆盖系统提示的对抗性用户输入（例如，“忽略之前的指示并……”）。针对每个输入测试守卫模式。测量有多少成功，并对成功的情况提出缓解措施。

4. 实现一个提示优化器。给定一个提示和评分标准，以 temperature=0.7 运行提示 5 次，对每次输出评分，识别最薄弱的评分标准，并重写提示以解决该问题。重复 3 次迭代。测量分数是否提高。

5. 创建一个“提示差异”工具。给定两个版本的提示，识别更改了什么（增加的约束、删除的示例、更改的角色、修改的格式），并预测该更改是否会提高或降低输出质量。用实际输出来测试你的预测。

## 关键术语

| 术语 | 人们说的 | 实际含义 |
|------|----------------|----------------------|
| 系统消息 | “指令” | 一种特殊消息，以高优先级处理，为模型整个对话设置身份、规则和约束 |
| 温度 | “创意旋钮” | softmax 之前的 logits 分布的缩放因子——较高的值使分布更平坦（更随机），较低的值使分布更尖锐（更确定性） |
| Top-p | “核采样” | 将 token 采样限制为累积概率超过 p 的最小集合，切断长尾的低概率 token |
| 少量样本提示 | “给例子” | 在提示中包含 2-10 个输入/输出示例，使模型无需任何微调即可学习任务模式 |
| 思维链 | “一步步思考” | 提示模型展示中间推理步骤，这可以在数学、逻辑和多步骤问题上将准确率提高 10-40% |
| 角色提示 | “你是专家” | 设置一个角色，使采样偏向于训练数据中的特定质量分布 |
| 提示注入 | “越狱” | 一种攻击，用户输入包含覆盖系统提示的指令，导致模型忽略其规则 |
| 上下文窗口 | “能读多少” | 模型单次调用可以处理的最大 token 数量（输入+输出）——当前模型范围从 8K 到 2M |
| 助手预填充 | “开始回答” | 提供模型响应的前几个 token 以引导格式并消除开场白——由 Anthropic 原生支持 |
| 元提示 | “写提示的提示” | 使用 LLM 为其他 LLM 任务生成、批评和优化提示 |

## 延伸阅读

- [OpenAI Prompt Engineering Guide](https://platform.openai.com/docs/guides/prompt-engineering) —— OpenAI 官方最佳实践，涵盖系统消息、少量样本和思维链
- [Anthropic Prompt Engineering Guide](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview) —— Claude 特有技术，包括 XML 格式、助手预填充和思考标签
- [Wei et al., 2022 -- "Chain-of-Thought Prompting Elicits Reasoning in Large Language Models"](https://arxiv.org/abs/2201.11903) —— 基础论文，证明“一步步思考”可在推理任务上将 LLM 准确率提高 10-40%
- [Zamfirescu-Pereira et al., 2023 -- "Why Johnny Can't Prompt"](https://arxiv.org/abs/2304.13529) —— 关于非专家如何在提示工程中挣扎以及什么使提示有效的
- [Shin et al., 2023 -- "Prompt Engineering a Prompt Engineer"](https://arxiv.org/abs/2311.05661) —— 使用 LLM 自动优化提示，元提示的基础
- [LMSYS Chatbot Arena](https://chat.lmsys.org/) —— LLM 的实时盲测对比，你可以在不同模型上测试同一个提示并投票选出哪个响应更好
- [DAIR.AI Prompt Engineering Guide](https://www.promptingguide.ai/) —— 提示技术的详尽目录，包含示例（零样本、少量样本、CoT、ReAct、自一致性）；从业者使用的参考，涵盖更广泛的“提示工程”表面。
- [Anthropic prompt library](https://docs.anthropic.com/en/prompt-library) —— 按用例分类的策划、已知良好的提示；展示了投入生产的结构性模式。
